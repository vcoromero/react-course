...
export const ScrambleWords = () => {
    ...
    const handleGuessSubmit = (e: React.FormEvent) => {
        ...
        setPoints(points + 1)
        setGuess('')
        if (points === GAME_WORDS.length) setIsGameOver(true)

        if (words.length !== 0) {
            const unGuessWords = words.filter(word => word !== guess)
            setWords(unGuessWords)
            setCurrentWord(unGuessWords[0])
            setScrambledWord(scrambleWord(unGuessWords[0]))
        }
